...
describe('CustomLogoComponent', () => {
  ...
  const setupTest = (logoConfig: any) => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [{provide: THEME_SERVICE, useClass: MockThemeService}],
    }).compileComponents();
  ...
  describe('with valid logo config', () => {
    ...
    it('should render the logo with accessible text', () => {
      const linkElement = fixture.debugElement.query(By.css('a'));
      const imgElement = fixture.debugElement.query(By.css('img'));

      expect(imgElement.nativeElement.src).toContain('test.png');
      expect(linkElement.nativeElement.getAttribute('aria-label')).toBe('Test Logo');
      expect(imgElement.nativeElement.alt).toBe('Test Logo');
    });
  });
